...
class MeetingEntryFragment  
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        (activity as AppCompatActivity).supportActionBar?.title = "Schedule a meeting"

        meetingEntryBinding.fabDone.setOnClickListener { presenter.saveMeeting() }
    ...
    private fun createTabIcons() {
        ...
        meetingEntryBinding.tabLayout.getTabAt(1)?.setIcon(R.drawable.ic_group_add_white_24dp)
        meetingEntryBinding.tabLayout.getTabAt(1)?.text = "Attendees"
        meetingEntryBinding.tabLayout.getTabAt(1)?.customView = tabTwo

        val tabThree = LayoutInflater.from(context).inflate(R.layout.custom_tab, null)
        meetingEntryBinding.tabLayout.getTabAt(2)?.setIcon(R.drawable.ic_attach_file_black_24dp)
        meetingEntryBinding.tabLayout.getTabAt(2)?.text = "Attachments"
        meetingEntryBinding.tabLayout.getTabAt(2)?.customView = tabThree
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            android.R.id.home -> {
                NavHostFragment.findNavController(this).navigate(R.id.timeTableFragment)
                return true
            }
